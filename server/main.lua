...
function CheckPlayerMoney(source)
    ...
    elseif Config.Framework == 'qb' then
        ...
        local cashCount = exports.ox_inventory:GetItemCount(source, Config.OxInventoryCashItem) or 0
        if cashCount >= Config.Cost then
            return true
        end
    end
...
function ProcessPayment(source)
    ...
    elseif Config.Framework == 'qb' then
        ...
        if exports.ox_inventory:GetItemCount(source, Config.OxInventoryCashItem) >= Config.Cost then
            exports.ox_inventory:RemoveItem(source, Config.OxInventoryCashItem, Config.Cost)
            paymentMethod = 'cash'
            paid = true
        end
    end
